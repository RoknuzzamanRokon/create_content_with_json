...
def get_data_from_json_file(file_name):
    folder_path = "D:/hotels_content_to_create_json_file/HotelInfo/Paximum"
...
def insert_data_to_inno_table_with_tracking(engine, table, tracking_file_path):
    ...
    if not tracking_ids:
        directory = "D:/hotels_content_to_create_json_file/HotelInfo/Paximum"
        file_path = "paximum_hotel_id_lsit.txt"
    ...
    for hotel_id in tracking_ids.copy():
        try:
            with engine.connect() as conn:
                query = text(f"SELECT COUNT(1) FROM {table.name} WHERE HotelId = :hotel_id AND SupplierCode = 'Paximum'")
