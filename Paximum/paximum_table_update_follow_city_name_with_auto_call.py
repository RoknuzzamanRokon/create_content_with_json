...
def process_cities():
    ...
    while remaining_cities:
        if datetime.now() - start_time > max_duration:
            logging.info("Exceeded 9 hours of runtime. Exiting for restart.")
            return  
...
while True:
    process_cities()
    remaining_cities = read_tracking_file("update_city_info.txt")
    if not remaining_cities:  
        logging.info("All cities processed. Exiting program.")
        break
    logging.info("Restarting process for remaining cities.")
    time.sleep(10)
